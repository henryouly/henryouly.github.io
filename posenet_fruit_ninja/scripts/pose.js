(function (global) {
     ...
     */
    async function drawSkeleton(keypoints, minConfidence, ctx, scale = 1, color='cyan') {
        const adjacentKeyPoints = await posenet.getAdjacentKeyPoints(keypoints, minConfidence);
    ...
    // kick off the demo
    bindPage();
    // tf.setBackend('webgl').then(() => bindPage());
